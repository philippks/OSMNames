...
def export_geonames():
    ...
    gzip_tsv_file(geonames_export_path())
...
def export_housenumbers():
    ...
    gzip_tsv_file(housenumbers_export_path())
...
def gzip_tsv_file(tsv_file_path):
    with open(tsv_file_path, "rb") as f_in, gzip.open(
        "{}.gz".format(tsv_file_path), "wb"
    ) as f_out:
