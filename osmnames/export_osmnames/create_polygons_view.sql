...
SELECT
  ...
  type,
  round(ST_X(ST_PointOnSurface(ST_Buffer(geometry, 0.0)))::numeric::numeric, 7) AS lon,
  round(ST_Y(ST_PointOnSurface(ST_Buffer(geometry, 0.0)))::numeric::numeric, 7) AS lat,
  place_rank,
